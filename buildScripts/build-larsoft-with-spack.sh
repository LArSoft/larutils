...
mkdir -p ${blddir}/packages || exit 1

buildFW="${working_dir}/artutilscripts/tools/buildFW"
tensorflow_spack_install="${working_dir}/artutilscripts/tools/tensorflow_spack_install.sh"
...
echo "begin build"
echo
if [ -d ${blddir}/packages/spack ]; then
  echo "spack packages already exist"
else
  "${tensorflow_spack_install}" packages
fi
export PRODUCTS=${blddir}:${blddir}/packages
(( ${#labels[@]} > 0 )) && lopt=-l
